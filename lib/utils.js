...
// Basket et Natation ont leur propre page dédiée (app/(protected)/basket,
// app/(protected)/natation) et ne passent donc jamais par la route
// générique [sportSlug]. Cette liste ne sert qu'à autoriser d'éventuels
// futurs sports "principaux" à utiliser le gabarit générique — pour
// l'instant vide : tout sport hors Basket/Natation est un "autre sport" et
// n'apparaît que sous /autres-sports (voir la colonne is_main en base).
export const MAIN_SPORT_SLUGS = [];

// URL publique d'un asset FFBB (écusson de club, logo de poule...). Pas
// besoin d'authentification côté client — c'est un endpoint public de
// l'API FFBB (Directus).
export function ffbbAssetUrl(assetId, { width } = {}) {
  if (!assetId) return null;
  const qs = width ? `?width=${width}` : "";
  return `https://api.ffbb.com/assets/${assetId}${qs}`;
}
...
// Libellé de saison basket ("2026-2027") utilisé pour rattacher chaque match
// et chaque ligne de classement à une saison, afin de garder l'historique des
// saisons précédentes en base sans jamais les écraser lors d'une resynchro.
export function computeCurrentSeasonLabel() {
  const endYear = computeCurrentSeasonYear();
